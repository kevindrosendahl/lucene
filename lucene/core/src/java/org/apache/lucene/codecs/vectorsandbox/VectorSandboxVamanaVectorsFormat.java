...
public final class VectorSandboxVamanaVectorsFormat extends KnnVectorsFormat {
  ...
  static final String META_CODEC_NAME = "VectorSandboxVamanaVectorsFormatMeta";
   ...
   * {@link VectorSandboxVamanaVectorsFormat#DEFAULT_MAX_CONN}. See {@link HnswGraph} for more
   * details.
   */
  ...
  public VectorSandboxVamanaVectorsFormat() {
    this(DEFAULT_MAX_CONN, DEFAULT_BEAM_WIDTH, DEFAULT_ALPHA);
  }
  ...
  public VectorSandboxVamanaVectorsFormat(int maxConn, int beamWidth, float alpha) {
    ...
    if (!Float.isFinite(alpha) || Float.compare(1.0f, alpha) > 1) {
      throw new IllegalArgumentException("alpha must be finite and greater than or equal to 1.0");
    }
  ...
  public KnnVectorsWriter fieldsWriter(SegmentWriteState state) throws IOException {
    return new VectorSandboxVamanaVectorsWriter(state, maxConn, beamWidth, alpha);
  }
