...
public class HnswGraphSearcher<T> {
   ...
   * @param similarityFunction the similarity function to compare vectors
   * @param graph              the graph values. May represent the entire graph, or a level in a
   *                           hierarchical graph.
   * @param acceptOrds         {@link Bits} that represents the allowed document ordinals to match,
   *                           or {@code null} if they are all allowed to match.
  ...
  public static NeighborQueue search(
    ...
    try (Arena arena = Arena.openConfined()) {
      MemorySegment queryMemory = arena.allocateArray(ValueLayout.JAVA_FLOAT, query);
      ...
      return search(query, queryMemory, topK, vectors, graph, graphSearcher, acceptOrds,
          visitedLimit);
    }
  }
  ...
  public static NeighborQueue search(
    ...
    OnHeapHnswGraphSearcher<float[]> graphSearcher =
        new OnHeapHnswGraphSearcher<>(
            ...
            new NeighborQueue(topK, true),
            new SparseFixedBitSet(vectors.size()));
    return search(query, null, topK, vectors, graph, graphSearcher, acceptOrds, visitedLimit);
  }
  ...
  private static <T> NeighborQueue search(
    ...
    for (int level = graph.numLevels() - 1; level >= 1; level--) {
      results = graphSearcher.searchLevel(query, queryMemory, 1, level, eps, vectors, graph, null,
          visitedLimit);
    ...
    results =
        graphSearcher.searchLevel(query, queryMemory, topK, 0, eps, vectors, graph, acceptOrds,
            visitedLimit);
  ...
  public NeighborQueue searchLevel(
      ...
      HnswGraph graph)
      throws IOException {
    return searchLevel(query, queryMemory, topK, level, eps, vectors, graph, null,
        Integer.MAX_VALUE);
  }
  ...
  private float compare(T query, MemorySegment queryMemory, RandomAccessVectorValues<T> vectors,
   ...
   * before calling this method. The default implementation will just call
   * {@link HnswGraph#nextNeighbor()}
   *
