...
public class KMeansPlusPlus {
  ...
  public static float[][] cluster(float[][] points, int k, VectorSimilarityFunction similarityFunction, Random random, int maxIterations) {
    if (k <= 0 || k > points.length) {
      throw new IllegalArgumentException(String.format("invalid number of clusters: %d", k));
    }

    KMeansPlusPlus kMeans = new KMeansPlusPlus(points, k, similarityFunction, random);
    return kMeans.run(maxIterations);
  }
  ...
  private KMeansPlusPlus(float[][] points, int k, VectorSimilarityFunction similarityFunction, Random random) {
    this.points = points;
    this.k = k;
